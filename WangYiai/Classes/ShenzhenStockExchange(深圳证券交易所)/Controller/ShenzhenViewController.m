...
@interface ShenzhenViewController ()<UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate>
...
//    [HttpTool postWithPath:@"/finance/stock/szall" params:@{@"key":AppKey,@"type":@1,@"page":@1} success:^(id json) {
...
//        [weakSelf ShowHint:@"请检查你的网络" yOffset:0];
//    }];
//    return;
    
    for (int i = 1; i < 29; i++) {
        [HttpTool postWithPath:@"/finance/stock/szall" params:@{@"key":AppKey,@"type":@4,@"page":@(i)} success:^(id json) {
            if ([json[@"reason"] isEqualToString:@"SUCCESSED!"]) {
                [Arr addObjectsFromArray:json[@"result"][@"data"]];
                if (Arr.count >= 2239) {
                    [weakSelf dataBase:Arr];
